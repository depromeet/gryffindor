...
const MOCK_DATA = {
  ...
  phone: "02-1234-5678",
  menus: [
    { name: "대표 메뉴 1", price: 12000, isRepresentative: true, imageUrl: "" },
    { name: "일반 메뉴 1", price: 9000, isRepresentative: false, imageUrl: "" },
    { name: "일반 메뉴 2", price: 8000, isRepresentative: false, imageUrl: "" },
  ],
  seatInfo: {
    cubicle: true,
    barTable: false,
    ...
    <TransitionLayout>
      ...
      <article className="mt-5 flex w-full flex-col gap-3 pl-5">
        ...
        <div className="flex flex-col gap-3">
          {MOCK_DATA.menus.map((menu) => (
            <MenuItem key={menu.name} {...menu} />
          ))}
        </div>
      </article>
      <SeatInfoSection seatInfo={MOCK_DATA.seatInfo} />
      <SeatImageGallery
        storeName={MOCK_DATA.name}
        level={MOCK_DATA.level}
        userName="홍길동"
        images={MOCK_DATA.seatImages}
      />
    </TransitionLayout>
  );
}
